...
const VideoManagement: React.FC = () => {
  ...
  const fetchVideos = async () => {
    try {
      const response = await authApi.get("/api/videos/creator/my-videos");
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authApi.put(`/api/videos/${editingVideo.id}`, {
  ...
  const handleDelete = async (video: Video) => {
    ...
    try {
      await authApi.delete(`/api/videos/${video.id}`);
      await fetchVideos();
    } catch (error: any) {
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-300 space-y-1">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span
                            className={`px-2 py-1 rounded text-xs ${getAgeRatingColor(
                              video.age_rating
                            )}`}
                          >
                            {video.age_rating}
