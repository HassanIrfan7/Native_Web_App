...
const FeaturedHomePage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [videosRes, statsRes] = await Promise.all([
        axios.get(`${BACKEND_URL}/api/videos?limit=8`),
  ...
  const VideoCard: React.FC<{ video: Video; featured?: boolean }> = ({
    video,
    featured = false,
  }) => (
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="bg-white bg-opacity-20 backdrop-blur-sm rounded-full p-4 group-hover:bg-opacity-30 transition-all duration-300 group-hover:scale-110">
              <Play
                className={`${featured ? "h-12 w-12" : "h-8 w-8"} text-white`}
              />
            </div>
          </div>
          ...
          {/* Age Rating Badge */}
          <div
            className={`absolute top-4 right-4 px-2 py-1 rounded text-xs font-medium ${getAgeRatingColor(
              video.age_rating
            )}`}
          >
            {video.age_rating}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <div className="text-center">
            ...
            <p className="text-xl md:text-2xl mb-8 max-w-3xl mx-auto opacity-90">
              Join millions of creators and viewers sharing stories,
              experiences, and creativity on AzureStream.
            </p>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Hand-picked videos from our top creators
            </p>
          </div>
          <Link
            to="/browse"
